...
| `treasurydirect` | 미 국채 입찰 (응찰률·낙찰금리·간접비중) | 키 불필요. 명목 이표물만 |
| `fiscaldata` | 유통국채 잔액(MSPD)·TGA 잔고(DTS) | 키 불필요. **지금 yaml 에서 주석 처리** |
...
- `Y_BAND` 세로축 정상범위 띠 (`{ from, to, label }`)
- 차트에 숫자를 얹는 것보다 **옆에 작은 표**를 붙이는 쪽이 낫다. 선 끝에 값을 그리면
  선을 가리고 겹칠 때 못 읽는다 (국채 입찰 차트에서 한 번 해봤다가 되돌렸다)
...
**하다 만 것**
- **QRA(미국 국채 수급) 화면.** 수집기 `fiscaldata.py` 는 완성(유통잔액 종류별·Bill 비중·
  TGA 일별)이고 `indicators.yaml` 의 `us_qra` 만 주석 처리해 뒀다 — 화면이 없어서다.
  만들 것: 순발행(유통잔액 분기 변화, 종류별 누적막대) / Bill 비중 + TBAC 권고대역
  (`Y_BAND`, **대역 수치는 TBAC 문서로 확인한 뒤** 그릴 것) / TGA 경로 / 분기 발행 전망
  (us_auction 예정 합계, 추정분 구분). QRA 의 순발행 **전망치**는 보도자료 본문이라
  API 가 없다 — 넣으려면 yaml 수동 입력칸이 필요하다.
