...
// min: 차트만 있는 화면은 마크업이 짧아 기본 2000자를 못 넘는다. 그런 화면만 낮춰 준다.
const chk = (name, elId, min = 2000) => {
...
// 화면 안에 특정 문구가 있는지 (분해 표가 실제로 붙었는지 같은 것)
const chkHas = (name, elId, needle) => {
  const el = w.document.getElementById(elId);
  const ok = !!el && el.innerHTML.includes(needle);
  if (!ok) fail++;
  console.log('  ' + name.padEnd(26) + (ok ? '           OK' : '        ⚠ 없음: ' + needle));
};
...
(async () => {
  ...
  if (w.__MACRO__ && w.__MACRO__.us_pce) {
    await ev('enterUsPce()');
    for (const t of ['price', 'real', 'income', 'mkt', 'sd']) { ev(`usPceTab='${t}'; refresh();`); chk('미국 PCE > ' + t, 'retailWrap'); }
